...
// 3-1. Add third map style as tile layer.
...
// 3-2. Create a base layer that holds all three maps.
let baseMaps = {
  "Streets": streets,
...
// 1-1., 2-1/ Add a 3rd layer group for the major earthquake data.
let allEarthquakes = new L.LayerGroup();
let tectonicPlates = new L.LayerGroup();
let majorEarthquakes = new L.LayerGroup();

// 1-2.,2-2. Add a reference to the major earthquake group to the overlays object.
let overlays = {
...
var melink = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson";
d3.json(melink, function(data) {
...
// 2-4. Use the same style as the earthquake data.
  function styleInfo(feature) {
    return {
      opacity: 1,
      fillOpacity: 1,
      fillColor: getmeColor(feature.properties.mag),
      color: "#000000",
      radius: getmeRadius(feature.properties.mag),
      stroke: true,
      weight: 0.5
    };
  }
...
// 2-6. Use the function that determines the radius of the earthquake marker based on its magnitude.
  function getmeRadius(magnitude) {
    if (magnitude === 0) {
      return 1;
    }
    return magnitude * 4;
